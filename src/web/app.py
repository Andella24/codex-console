...
def create_app() -> FastAPI:
    ...
    async def accounts_page(request: Request):
        ...
        if not _is_authenticated(request):
            return _redirect_to_login(request)
        return templates.TemplateResponse(name="accounts.html", request=request)
    ...
    async def email_services_page(request: Request):
        ...
        if not _is_authenticated(request):
            return _redirect_to_login(request)
        return templates.TemplateResponse(name="email_services.html", request=request)
    ...
    async def payment_page(request: Request):
        """支付页面"""
        return templates.TemplateResponse(name="payment.html", request=request)
